...
def get_datasets(imgs_dir,groundTruth_dir,borderMasks_dir,train_test="null"):
    ...
    for path, subdirs, files in os.walk(imgs_dir): #list all files, directories in the path
        for i in range(len(files)):
            ...
            border_masks_name = ""
            if train_test=="train":
                print 'working on: ', files[i][0:2]
                border_masks_name = '{:02}'.format(int(files[i][0:2])%20+1) + "_test_mask.gif"
    ...
    print groundTruth[0].shape, groundTruth.shape
    print "imgs max: " +str(np.max(imgs))
    print "imgs min: " +str(np.min(imgs))
    print "gT   max: " + str(np.max(groundTruth))
    print "gT   min: " + str(np.min(groundTruth))
    print "bm   max: " + str(np.max(border_masks))
    print "bm   min: " + str(np.min(border_masks))
